...
import "./table.css";
// import { requestOrderDetailInfo } from "service/platform";
// import { requestShopInfo } from "service/merchant";

import { Modal } from "antd";

const Print = (props) => {
    const { visible, orderDetailId, onCancel, onOk, userInfo } = props;
    ...
    useEffect(() => {
        if (orderDetailId) {
            // 根据子订单id获取打印信息，用于点餐台点餐清单子订单打印
            // requestOrderDetailInfo({ orderDetailId })
            //     .then(response => {
            //         setDetail(response);
            //         if (response.detailList) {
            //             setDetailSpu(response.detailList);
            //             // 计算数量总数和价格总数
        ...
        <div id="print">
                    ...
                    <tbody>
                        {
                            detailSpu.map((spu, index) => (
                                <tr key={index}>
                                    ...
                                    <th style={{ ...spuLine2Style, ...spuLineStyle }}>{spu.nowCount}</th>
                        ...
                        {/* <tr>
                            <th style={{ ...spuLine1Style, ...spuLineStyle }}>小计</th>
            ...
            {/* <div style={{
                ...
                <p style={fontStyle}>{t('layouts.Print.area')}<span style={{
                    float: "right"
                }}>{shopInfo.shopAddress}</span></p>
                <p style={fontStyle}>{t('layouts.Print.phone')}<span style={{
                    float: "right"
                }}>{shopInfo.contactPhone}</span></p>
            </div>
